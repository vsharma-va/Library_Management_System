...
{
	{
		std::string fileSaveLocation{ "Catalog.txt" };
		if (reload == true)
		{
			std::fstream catalog;
...
std::tuple<std::vector<std::string>, std::vector<std::string>> CreateAndSearchCatalog::searchBook(std::tuple<std::vector<std::string>, std::vector<int>> *catalog)
{
	// searches for the user requested book
	std::vector<std::string> perfectMatch{};
	std::vector<std::string> partialMatch{};
	for (std::string book : std::get<0>(*catalog))
	{
		...
		{
			match = true;
			perfectMatch.push_back(book);
		}
		else if ((where >= 0) && (!match))
		{
			match = false;
			partialMatch.push_back(book);
		}
	}
	return std::tuple<std::vector<std::string>, std::vector<std::string>> (perfectMatch, partialMatch);
}
